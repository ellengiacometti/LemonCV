...
def CrossTrataImagem(src,visual,verbose):
    ...
    _, s_Thresh = cv.threshold(s,50,255,cv.THRESH_BINARY)
    ...
    s_Result = cv.bitwise_and(cv.cvtColor(img, cv.COLOR_BGR2RGB), cv.cvtColor(img, cv.COLOR_BGR2RGB), mask=s_Thresh)
    """ PRINTANDO IMAGENS DO PROCESSO   """
    if visual==1:
        fig, ( ax2,ax4) = plt.subplots(1, 2, figsize=(10, 5), sharex=True, sharey=True)
        ax2.axis('off')
    ...
    BoundingBox[s_Thresh == 255] = img[s_Thresh == 255]
    # Definindo pontos para corte
    (x, y) = np.where(s_Thresh == 255)
    ...
    _, contornosCV, _ = cv.findContours(s_Thresh, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    ...
    histH = cv.calcHist([h], [0], s_Thresh, [256], [0, 256])
    histH = list(map(float, histH[0:255][:]))
    histS = cv.calcHist([s], [0], s_Thresh, [256], [0, 256])
    histS = list(map(float, histS[0:255][:]))
    histV = cv.calcHist([v], [0], s_Thresh, [256], [0, 256])
