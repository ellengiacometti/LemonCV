...
for filename in os.listdir(dir):
    ...
    for index, sample_filename in enumerate(sample_column):
        x=0
        if filename == os.listdir(dir)[0]:
            final_sample.append(sample_filename)
            maturity = final_sample.__str__()[-9]
            defect = final_sample.__str__()[-8]
...
raw_data = {'Sample': final_sample_name, 'R': R,'L': L,'C': C,'S': S,'X': X,'Voted Label':final_label}
